...
function JSONReader(object, optionalProperty) {
...
JSONReader.prototype._get = function(name) {
...
JSONReader.prototype._getCnd = function(name, defaultValue) {
  if (typeof name !== 'string') {
    throw new TypeError('First argument must be a string');
  }
  var value = this.object[name];
...
JSONReader.prototype.hasProperty = function (name) {
  if (typeof name !== 'string') {
...
JSONReader.prototype.getString = function(name) {
...
JSONReader.prototype.getStringConditional = function(name, defaultValue) {
...
JSONReader.prototype.getBinary = function(name) {
  return Base64URL.decode(this.getString(name));
};

JSONReader.prototype.getInteger = function(name) {
  var value = this._get(name);
...
JSONReader.prototype.getBoolean = function(name) {
  var value = this._get(name);
...
JSONReader.prototype.getArray = function(name) {
  var value = this._get(name);
...
JSONReader.prototype.getObject = function(name) {
  var value = this._get(name);
  if (typeof value !== 'object' || Array.isArray(value)) {
...
JSONReader.prototype.checkForUnread = function() {
