...
function EncryptedData(reader) {
  ...
  if (reader.hasProperty(KEY_ENCRYPTION_JSON)) {
    ...
    this.keyEncryptionAlgorithm = keyEncryption.getString(Jcs.ALGORITHM_JSON);
    if (keyEncryption.hasProperty(Jcs.PUBLIC_KEY_JSON)) {
      this.publicKey = keyEncryption.getPublicKey();
    } else {
      this.keyId = keyEncryption.getStringConditional(Jcs.KEY_ID_JSON);
    }
    ...
    this.keyId = reader.getStringConditional(Jcs.KEY_ID_JSON);
  }
