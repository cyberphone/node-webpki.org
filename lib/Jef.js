...
function EncryptedData(reader) {
  ...
  if (reader.hasProperty(ENCRYPTED_KEY_JSON)) {
    ...
    this.authenticatedData = encryptedKey.getNormalizedData();
    ...
    this.authenticatedData = ByteArray.stringToUtf8(this.dataEncryptionAlgorithm);
  }
...
function encode(encryptionObject,
  ...
  if (keyEncryptionKey_or_keyId instanceof Keys.PublicKey) {
    ...
    if (isRsaKey(keyEncryptionAlgorithm_or_dataEncryptionKey)) {
      ...
      encryptedKey.setObject(EPHEMERAL_KEY_JSON).setCorePublicKey(ecdhRes.publicKey);
    }
    authenticatedData = encryptedKey.getNormalizedData();
  } else {
    dataEncryptionKey = keyEncryptionAlgorithm_or_dataEncryptionKey;
    authenticatedData = ByteArray.stringToUtf8(dataEncryptionAlgorithm);
