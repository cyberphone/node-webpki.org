...
function EncryptedData(reader) {
  ...
  // Get a canonicalized JSON string (=authenticated data).
  this.authenticatedData = JsonUtil.getCanonicalizedData(clone);
  ...
  if (reader.hasProperty(ENCRYPTED_KEY_JSON)) {
    ...
    var keyEncryption = reader.getObject(ENCRYPTED_KEY_JSON);
    ...
    if (isRsaAlgorithm(this.keyEncryptionAlgorithm)) {
      this.encryptedKeyData = keyEncryption.getBinary(CIPHER_TEXT_JSON);
    } else {
      if (this.keyEncryptionAlgorithm != JOSE_ECDH_ES_ALG_ID) {
        throw new TypeError('Unsupported algorithm: ' + this.keyEncryptionAlgorithm);
      }
