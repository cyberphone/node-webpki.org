...
DecodedSignature.prototype.getKeyId = function() {
  return this.keyId;
}
...
Verifier.prototype.decodeSignature = function(signedObject) {
  ...
  if (ALGORITHMS[findAlgorithm(signatureAlgorithm) + 1]) {
    ...
    if (clone.certificatePath === undefined) {
      ...
      var signatureCertificate = decodedSignature.certificatePath[0];
      publicKey = signatureCertificate.getPublicKey();
      if (clone.signerCertificate) {
        var signerCertificate = Util.getObjectUnconditionally(clone, 'signerCertificate');
        if (Util.getStringUnconditionally(signerCertificate,'issuer') !=
              signatureCertificate.getIssuer() ||
            Util.getStringUnconditionally(signerCertificate,'serialNumber') !=
              signatureCertificate.getSerialNumber().toString() ||
            Util.getStringUnconditionally(signerCertificate,'subject') !=
              signatureCertificate.getSubject()) {
          throw new TypeError('Non-matching "signerCertificate" properties');
        }
        Util.checkForUnexpected(clone.signerCertificate, 3);
        propertyCount++;
      }
    }
