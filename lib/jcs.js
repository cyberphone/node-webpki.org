...
Signer.prototype.sign = function(object2Sign) {
  ...
  signatureWriter.setString('algorithm', this.signatureAlgorithm);
  if (this.keyId !== undefined) {
    signatureWriter.setString('keyId', this.keyId);
  }
  if (this.signatureKey instanceof Keys.PrivateKey) {
    if (this.certificatePath === undefined) {
      ...
      if (this.signerCertificateFlag) {
        signatureWriter.setObject('signerCertificate', new JsonUtil.ObjectWriter()
          .setString('issuer', this.certificatePath[0].getIssuer())
          .setBigInteger('serialNumber', this.certificatePath[0].getSerialNumber())
          .setString('subject', this.certificatePath[0].getSubject()));
      }
      var arrayWriter = signatureWriter.setArray('certificatePath');
      this.certificatePath.forEach((certificate) => {
        arrayWriter.setBinary(certificate.getCertificateBlob());
      });
    }
  }
  signatureWriter.setBinary('value', this._localSigner(writer.getNormalizedData()));
  return object2Sign;
};
...
Verifier.prototype.decodeSignature = function(signedObject) {
  ...
  var reader = new JsonUtil.ObjectReader(signedObject, 'signature');
  // Clone "signature" child object
  var clone = Object.assign({}, signedObject.signature);
  // Get signature value
  var signatureValue = reader.getBinary('value');
  ...
  var signedData = new JsonUtil.ObjectReader(signedObject).getNormalizedData();
  // Restore signed object
  signedObject.signature = clone;
  // We should always have an algorithm property
  decodedSignature.signatureAlgorithm = reader.getString('algorithm');
  // An optional "keyId" should be catered for
  if (clone.keyId !== undefined) {
    decodedSignature.keyId = reader.getString('keyId');
  }
  ...
  if (ALGORITHMS[findAlgorithm(decodedSignature.signatureAlgorithm) + 1]) {
    ...
    if (clone.certificatePath === undefined) {
      decodedSignature.publicKey = publicKey = Keys.encodePublicKey(reader.getObject('publicKey'));
    } else {
      var arrayReader = reader.getArrayReader('certificatePath');
      ...
      if (clone.signerCertificate !== undefined) {
        var certPropReader = reader.getObjectReader('signerCertificate');
        if (certPropReader.getString('issuer') != signatureCertificate.getIssuer() ||
            !certPropReader.getBigInteger('serialNumber').equals(signatureCertificate.getSerialNumber()) ||
            certPropReader.getString('subject') != signatureCertificate.getSubject()) {
