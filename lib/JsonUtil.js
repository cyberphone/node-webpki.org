...
function _encodeEncryptedObject(unencryptedData,
  ...
  if (keyEncryptionAlgorithm) {
    var encryptedKey = encryptionObject.setObject(Jef.KEY_ENCRYPTION_JSON)
      .setString(Jsf.ALGORITHM_JSON, keyEncryptionAlgorithm);
    if (keyId) {
      encryptedKey.setString(Jsf.KEY_ID_JSON, keyId);
    } else {
    ...
    if (Jef.isRsaAlgorithm(keyEncryptionAlgorithm)) {
      ...
      encryptedKey.setBinary(Jef.ENCRYPTED_KEY_JSON,
...
ObjectReader.prototype.getSignature = function(optionalSignatureLabel) {
  this._getObject(optionalSignatureLabel === undefined ? 
                                    Jsf.SIGNATURE_JSON : optionalSignatureLabel);
  return new Jsf.Verifier(optionalSignatureLabel).decodeSignature(this.object);
};
