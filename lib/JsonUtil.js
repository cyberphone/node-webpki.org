...
ObjectWriter.prototype.setCorePublicKey = function(publicKey) {
  if (publicKey instanceof Keys.PublicKey) {
    Object.keys(publicKey.jcs).forEach((key) => {
      this._setProperty(key, publicKey.jcs[key]);
    });
    return this;
  }
...
ObjectWriter.prototype.setPublicKey = function(publicKey) {
  return this.setObject(Jcs.PUBLIC_KEY_JSON).setCorePublicKey(publicKey);
};
...
ObjectWriter.prototype.setEncryptionObject = function(unencryptedData,
                                                      dataEncryptionAlgorithm,
                                                      keyEncryptionKey_or_keyId,
                                                      keyEncryptionAlgorithm_or_dataEncryptionKey) {
  return Jef.encode(this,
                    unencryptedData,
                    dataEncryptionAlgorithm,
                    keyEncryptionKey_or_keyId,
                    keyEncryptionAlgorithm_or_dataEncryptionKey);
...
ObjectReader.prototype.getEncryptedObject = function() {
...
const JS_ID_PATTERN = new RegExp(/^[a-z,A-Z,$,_]+[a-z,A-Z,$,_,0-9]*$/);
