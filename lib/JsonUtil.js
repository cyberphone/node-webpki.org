...
function cleanObject(object) {
   if (object instanceof ObjectWriter || object instanceof ObjectReader) {
     return object.object;
   }
   return object;
}

function getCanonicalizedData(object) {
  var buffer = '';
  serialize(object);
  return ByteArray.stringToUtf8(buffer);
...
ObjectWriter.setCorePublicKey = function(publicKey) {
  ...
  if (publicKey instanceof Keys.PublicKey) {
    Object.keys(publicKey.jwk).forEach((key) => {
      writer._setProperty(key, publicKey.jwk[key]);
    });
    return writer;
  }
...
ObjectWriter.prototype.setPublicKey = function(publicKey) {
  return this.setObject(Jsf.PUBLIC_KEY_JSON, ObjectWriter.setCorePublicKey(publicKey));
};

ObjectWriter.prototype.setSignature = function(signer, optionalSignatureLabel) {
  signer.sign(this.object, optionalSignatureLabel);
  return this;
};
...
function _encodeEncryptedObject(unencryptedData,
                                ...
                                dataEncryptionKey) {
  var encryptionObject = new ObjectWriter().setString(Jsf.ALGORITHM_JSON, dataEncryptionAlgorithm)
  if (keyEncryptionAlgorithm) {
    var keyEncryption = encryptionObject.setObject(Jef.KEY_ENCRYPTION_JSON)
      .setString(Jsf.ALGORITHM_JSON, keyEncryptionAlgorithm);
    if (keyId) {
      keyEncryption.setString(Jsf.KEY_ID_JSON, keyId);
    } else {
      keyEncryption.setPublicKey(keyEncryptionKey);
    }
    if (Jef.isRsaAlgorithm(keyEncryptionAlgorithm)) {
      dataEncryptionKey = Jef.generateDataEncryptionKey(dataEncryptionAlgorithm);
      keyEncryption.setBinary(Jef.ENCRYPTED_KEY_JSON,
      ...
      keyEncryption.setObject(Jef.EPHEMERAL_KEY_JSON, ObjectWriter.setCorePublicKey(ecdhRes.publicKey));
    }
  } else {
    if (keyId) {
      encryptionObject.setString(Jsf.KEY_ID_JSON, keyId)
    }
  }
  var result = Jef.contentEncryption(dataEncryptionAlgorithm,
                                     dataEncryptionKey,
                                     unencryptedData,
                                     encryptionObject.getCanonicalizedData());
...
ObjectWriter.prototype.getCanonicalizedData = function() {
  return getCanonicalizedData(this.object);
};
...
ObjectReader.prototype.getPublicKey = function() {
  return this.getObject(Jsf.PUBLIC_KEY_JSON).getCorePublicKey();
};

ObjectReader.prototype.getSignature = function() {
  this._getObject(Jsf.SIGNATURE_JSON);
  return new Jsf.Verifier(true).decodeSignature(this.object);
};
...
ObjectReader.prototype.getCanonicalizedData = function() {
  return getCanonicalizedData(this.object);
};
...
// Methods
exports.getCanonicalizedData = getCanonicalizedData;
