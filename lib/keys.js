...
function encodeECPublicKey(jcs) {
  ...
  var paramsEntry = getECParamsFromCurve(Util.getPropertyUnconditionally(jcs,'curve'));
  var xValue = Base64URL.decode(Util.getPropertyUnconditionally(jcs, 'x'));
  var yValue = Base64URL.decode(Util.getPropertyUnconditionally(jcs, 'y'));
...
function encodeRSAPublicKey(jcs) {
  ...
  var nValue = Base64URL.decode(Util.getPropertyUnconditionally(jcs, 'n'));
  var eValue = Base64URL.decode(Util.getPropertyUnconditionally(jcs, 'e'));
  return new PublicKey(
    jcs,
    new ASN1.Encoder(ASN1.TAGS.SEQUENCE,
                     new ASN1.Encoder(ASN1.TAGS.SEQUENCE,
...
function encodePublicKey(jcs) {
  var type = Util.getPropertyUnconditionally(jcs,'type');
  if (type == 'RSA') {
